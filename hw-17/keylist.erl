...
init([]) ->
  ...
  {ok, #keylist_state{counter = 0}}.

handle_call({add, Key, Value, Comment}, _From, #keylist_state{counter = Counter} = State) ->
  ...
  NewState = State#keylist_state{counter = Counter + 1},
  {reply, {ok, NewState#keylist_state.counter},  NewState};

handle_call({delete, Key}, _From, #keylist_state{counter = Counter} = State) ->
...
handle_call({is_member, Key}, _From, #keylist_state{counter = Counter} = State) ->
...
handle_call({take, Key}, _From, #keylist_state{counter = Counter} = State) ->
  case ets:lookup(?TABLE_NAME, Key) of
    [] ->
      io:format("Element with the key ~p doesn`t exist in ETS~n", [Key]),
      {reply, false, State};
...
handle_call({find, Key}, _From, #keylist_state{counter = Counter} = State) ->
  case ets:lookup(?TABLE_NAME, Key) of
    [] ->
...
handle_call({match, Pattern}, _From, #keylist_state{counter = Counter} = State) ->
  MatchResult = ets:match(?TABLE_NAME, Pattern),
  NewState = State#keylist_state{counter = Counter + 1},
  {reply, {ok, MatchResult}, NewState};

handle_call({match_object, Pattern}, _From, #keylist_state{counter = Counter} = State) ->
  MatchResult = ets:match_object(?TABLE_NAME, Pattern),
  NewState = State#keylist_state{counter = Counter + 1},
  {reply, {ok, MatchResult}, NewState};

handle_call({select, Filter}, _From, #keylist_state{counter = Counter} = State) ->
